...
type mapInterfaceIntInt mapInterface[int, int]
type bench struct {
	setup func(*testing.B, mapInterfaceIntInt)
	perG  func(b *testing.B, pb *testing.PB, i int, m mapInterfaceIntInt)
}

func benchMap(b *testing.B, bench bench) {
	for _, m := range [...]mapInterfaceIntInt{&DeepCopyMap[int, int]{}, &RWMutexMap[int, int]{}, &SmartMutexMap[int, int]{}, &SyncMapClassic[int, int]{}, &SyncMap[int, int]{}} {
		b.Run(fmt.Sprintf("%T", m), func(b *testing.B) {
			m = reflect.New(reflect.TypeOf(m).Elem()).Interface().(mapInterfaceIntInt)
			if bench.setup != nil {
				bench.setup(b, m)
			}
...
func BenchmarkLoadMostlyHits(b *testing.B) {
	...
	benchMap(b, bench{
		setup: func(_ *testing.B, m mapInterfaceIntInt) {
			for i := 0; i < hits; i++ {
				m.LoadOrStore(i, i)
			}
...
func BenchmarkLoadOrStoreBalanced(b *testing.B) {
	...
	benchMap(b, bench{
		setup: func(b *testing.B, m mapInterfaceIntInt) {
