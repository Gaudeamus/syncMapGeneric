...
// the entry unchanged.
func (e *entry[V]) tryCompareAndSwap(expunged *V, old, new V) bool {
	p := e.p.Load()
	if p == nil || p == expunged {
		return false
	}
...
func (e *entry[V]) tryLoadOrStore(expunged *V, i V) (actual V, loaded, ok bool) {
	p := e.p.Load()
	if p == expunged {
	...
	for {
		var empty *V
		if e.p.CompareAndSwap(empty, &ic) {
			return i, false, true
		}
		p = e.p.Load()
		if p == expunged {
...
// unchanged.
func (e *entry[V]) trySwap(expunged *V, i *V) (*V, bool) {
	for {
		p := e.p.Load()
		if p == expunged {
			return nil, false
...
func (m *SyncMap[K, V]) Swap(key K, value V) (previous V, loaded bool) {
	read := m.loadReadOnly()
	if e, ok := read.m[key]; ok {
		if v, ok := e.trySwap(&m.expunged, &value); ok {
			if v == nil {
...
func (m *SyncMap[K, V]) CompareAndSwap(key K, old, new V) bool {
	read := m.loadReadOnly()
	if e, ok := read.m[key]; ok {
		return e.tryCompareAndSwap(&m.expunged, old, new)
	...
	swapped := false
	if e, ok := read.m[key]; ok {
		swapped = e.tryCompareAndSwap(&m.expunged, old, new)
	} else if e, ok := m.dirty[key]; ok {
		swapped = e.tryCompareAndSwap(&m.expunged, old, new)
